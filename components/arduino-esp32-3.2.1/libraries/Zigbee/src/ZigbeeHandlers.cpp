...
#if CONFIG_ZB_DELTA_OTA // Delta OTA, code is prepared for this feature but not
                        // enabled by default
...
#define OTA_ELEMENT_HEADER_LEN                                                 \
  6 /* OTA element format header size include tag identifier and length field  \
     */

/**
 * @name Enumeration for the tag identifier denotes the type and format of the
...
static esp_err_t
zb_attribute_reporting_handler(const esp_zb_zcl_report_attr_message_t *message);
...
static esp_err_t zb_cmd_ias_zone_enroll_response_handler(
    const esp_zb_zcl_ias_zone_enroll_response_message_t *message);
...
static esp_err_t zb_action_handler(esp_zb_core_action_callback_id_t callback_id,
  ...
  case ESP_ZB_CORE_IAS_ZONE_ENROLL_RESPONSE_VALUE_CB_ID:
    ret = zb_cmd_ias_zone_enroll_response_handler(
        (esp_zb_zcl_ias_zone_enroll_response_message_t *)message);
    break;
...
zb_attribute_set_handler(const esp_zb_zcl_set_attr_value_message_t *message) {
  ...
  log_v("Received message: endpoint(%d), cluster(0x%x), attribute(0x%x), data "
        "size(%d)",
        message->info.dst_endpoint, message->info.cluster,
        message->attribute.id, message->attribute.data.size);

  // List through all Zigbee EPs and call the callback function, with the
  // message
  for (std::list<ZigbeeEP *>::iterator it = Zigbee.ep_objects.begin();
    ...
    if (message->info.dst_endpoint == (*it)->getEndpoint()) {
      if (message->info.cluster == ESP_ZB_ZCL_CLUSTER_ID_IDENTIFY) {
        (*it)->zbIdentify(
            message); // method zbIdentify implemented in the common EP class
      } else {
        (*it)->zbAttributeSet(message); // method zbAttributeSet must be
                                        // implemented in specific EP class
      }
    }
  }
  return ESP_OK;
...
static esp_err_t zb_attribute_reporting_handler(
  ...
  log_v("Received report from address(0x%x) src endpoint(%d) to dst "
  ...
  for (std::list<ZigbeeEP *>::iterator it = Zigbee.ep_objects.begin();
    ...
    if (message->dst_endpoint == (*it)->getEndpoint()) {
      (*it)->zbAttributeRead(
          message->cluster, &message->attribute, message->src_endpoint,
          message->src_address); // method zbAttributeRead must be implemented
                                 // in specific EP class
    }
  }
  return ESP_OK;
}
...
static esp_err_t zb_cmd_read_attr_resp_handler(
  ...
  log_v("Read attribute response: from address(0x%x) src endpoint(%d) to dst "
        "endpoint(%d) cluster(0x%x)",
  ...
  for (std::list<ZigbeeEP *>::iterator it = Zigbee.ep_objects.begin();
    ...
    if (message->info.dst_endpoint == (*it)->getEndpoint()) {
      ...
      while (variable) {
        log_v("Read attribute response: status(%d), cluster(0x%x), "
        ...
        if (variable->status == ESP_ZB_ZCL_STATUS_SUCCESS) {
          if (message->info.cluster == ESP_ZB_ZCL_CLUSTER_ID_BASIC) {
            (*it)->zbReadBasicCluster(
                &variable->attribute); // method zbReadBasicCluster implemented
                                       // in the common EP class
          } else if (message->info.cluster == ESP_ZB_ZCL_CLUSTER_ID_TIME) {
            (*it)->zbReadTimeCluster(
                &variable->attribute); // method zbReadTimeCluster implemented
                                       // in the common EP class
          } else {
            (*it)->zbAttributeRead(
                message->info.cluster, &variable->attribute,
                message->info.src_endpoint,
                message->info.src_address); // method zbAttributeRead must be
                                            // implemented in specific EP class
          }
        }
...
static esp_err_t zb_configure_report_resp_handler(
  ...
  while (variable) {
    log_v("Configure report response: status(%d), cluster(0x%x), "
...
static esp_err_t zb_cmd_ias_zone_status_change_handler(
  ...
  log_v("IAS Zone Status Notification: from address(0x%x) src endpoint(%d) to "
...
static esp_err_t zb_window_covering_movement_resp_handler(
  ...
  log_v("Received message: endpoint(%d), cluster(0x%x), command(0x%x), "
        "payload(%d)",
  ...
  for (std::list<ZigbeeEP *>::iterator it = Zigbee.ep_objects.begin();
       it != Zigbee.ep_objects.end(); ++it) {
    if (message->info.dst_endpoint == (*it)->getEndpoint()) {
      (*it)->zbWindowCoveringMovementCmd(
          message); // method zbWindowCoveringMovementCmd must be implemented in
                    // specific EP class
    }
  }
  return ESP_OK;
...
static esp_err_t zb_ota_upgrade_status_handler(
  ...
  if (message->info.status == ESP_ZB_ZCL_STATUS_SUCCESS) {
...
#endif
      if (ret != ESP_OK) {
        log_e("Zigbee - Failed to begin OTA partition, status: %s",
              esp_err_to_name(ret));
        return ret;
      }
      break;
    case ESP_ZB_ZCL_OTA_UPGRADE_STATUS_RECEIVE:
      ...
      log_i("Zigbee - OTA Client receives data: progress [%ld/%ld]", offset,
            total_size);
      if (message->payload_size && message->payload) {
        uint16_t payload_size = 0;
        void *payload = NULL;
        ret = esp_element_ota_data(total_size, message->payload,
                                   message->payload_size, &payload,
                                   &payload_size);
        if (ret != ESP_OK) {
          log_e("Zigbee - Failed to element OTA data, status: %s",
                esp_err_to_name(ret));
          return ret;
        }
#if CONFIG_ZB_DELTA_OTA
        ret =
            esp_delta_ota_write(s_ota_handle, (uint8_t *)payload, payload_size);
#else
        ret = esp_ota_write(s_ota_handle, (const void *)payload, payload_size);
#endif
        if (ret != ESP_OK) {
          log_e("Zigbee - Failed to write OTA data to partition, status: %s",
                esp_err_to_name(ret));
          return ret;
        }
      }
    ...
    case ESP_ZB_ZCL_OTA_UPGRADE_STATUS_FINISH:
      ...
      log_i(
          "Zigbee - OTA Information: version: 0x%lx, manufacturer code: 0x%x, "
...
#endif
      if (ret != ESP_OK) {
        log_e("Zigbee - Failed to end OTA partition, status: %s",
              esp_err_to_name(ret));
        return ret;
      }
      ret = esp_ota_set_boot_partition(s_ota_partition);
      if (ret != ESP_OK) {
        log_e("Zigbee - Failed to set OTA boot partition, status: %s",
              esp_err_to_name(ret));
        return ret;
      }
...
static esp_err_t zb_ota_upgrade_query_image_resp_handler(
    const esp_zb_zcl_ota_upgrade_query_image_resp_message_t *message) {
  if (message->info.status == ESP_ZB_ZCL_STATUS_SUCCESS) {
    log_i("Zigbee - Queried OTA image from address: 0x%04hx, endpoint: %d",
          message->server_addr.u.short_addr, message->server_endpoint);
    log_i("Zigbee - Image version: 0x%lx, manufacturer code: 0x%x, image size: "
          "%ld",
    ...
    log_i("Zigbee - OTA image upgrade response status: 0x%x",
          message->info.status);
  }
  return ESP_OK;
}
...
static esp_err_t zb_cmd_default_resp_handler(
  ...
  log_v("Received default response: from address(0x%x), src_endpoint(%d) to "
        "dst_endpoint(%d), cluster(0x%x) with status 0x%x",
        message->info.src_address.u.short_addr, message->info.src_endpoint,
        message->info.dst_endpoint, message->info.cluster,
        message->status_code);
  return ESP_OK;
